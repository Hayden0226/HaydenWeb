...
- **Toodles 转换失败修复**：FFmpeg 转换失败（如内存不足）现在会正确报错并清理临时文件，不再显示「转换完成 (0 KB)」假成功；同时把易爆内存的默认值回调到稳定档（MP4→GIF 默认 720px/15fps，视频/图片缩放默认 1280px、裁剪默认 1080px），上限仍保留 4K/8192 可手动拉高。

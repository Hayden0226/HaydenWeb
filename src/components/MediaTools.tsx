...
const TOOL_GUIDES: Record<string, { title: string; items: { lead: string; text: string }[] }> = {
  ...
  'video-to-images': {
    title: 'Video → Images：按秒抽帧打包',
    items: [
      {
        lead: '这个工具做什么',
        text: '从视频里按时间抽出一张张图片并打包成 ZIP。适合做缩略图、关键帧预览、接触印样，或批量取帧给别的工具用。',
      },
      {
        lead: '帧数怎么算',
        text: '按「每秒 1 帧」抽取，最多帧数上限 500。帧数越多 ZIP 越大；长视频建议设小些（如 30–60），避免生成超大压缩包甚至内存不足。',
      },
      {
        lead: 'PNG 还是 JPG',
        text: 'PNG 无损、支持透明，但体积很大，适合需要后期或透明图的场景；JPG 有损、体积小，适合普通预览与分享。',
      },
      {
        lead: '质量不变',
        text: '抽出来的是原始视频帧，不会重新压缩，画质与视频当前质量一致；想更小唯有选 JPG 或降低尺寸。',
      },
      {
        lead: '实用建议',
        text: '短片段预览用 PNG、帧数小即可；长视频、只求快速预览推荐 JPG 并控制帧数。',
      },
    ],
  },
  'jpg-to-png': {
    title: 'JPG → PNG：为什么需要，以及不会发生什么',
    items: [
      {
        lead: '想清楚再转',
        text: 'JPG 是有损、PNG 是无损；但转 PNG 不会恢复 JPG 已丢失的细节，只是把剩下的像素「无损打包」进 PNG。',
      },
      {
        lead: '合适场景',
        text: '需要透明背景、需要无损再编辑、文字 / 线条 / 图标截图，或某软件只认 PNG 时，才值得转。',
      },
      {
        lead: '体积变化',
        text: 'PNG 面向无损，通常比 JPG 大很多；尤其照片、渐变画面会明显更大。',
      },
      {
        lead: '透明差距',
        text: 'JPG 本身不存透明通道；所以转 PNG 也不会凭空补出透明背景——原来半透明或透明的地方早已被填成实色。',
      },
      {
        lead: '实用建议',
        text: '纯粹为了缩小体积就别转 PNG（选 WebP 或更高质量 JPG）；真需要透明 / 无损 / 编辑再转。',
      },
    ],
  },
  'png-to-jpg': {
    title: 'PNG → JPG：注意透明会变白',
    items: [
      {
        lead: '格式差异',
        text: 'PNG 无损、支持透明；JPG 有损、不支持透明。转成 JPG 后，原本透明的区域会被填成白色（本工具即此行为）。',
      },
      {
        lead: '照片适合 JPG',
        text: 'JPG 对照片、渐变画面效率高、体积小，基本看不出差别，是网页分享照片的好选择。',
      },
      {
        lead: '素材/截图慎转',
        text: '文字、线条、图标、UI 截图转 JPG 后边缘易发糊、可能出现块状伪影；这类内容保留 PNG / WebP 更合适。',
      },
      {
        lead: '质量可调',
        text: '质量越低体积越小、但细节丢失越多；有损压缩不可逆，建议源图留一份无损备份。',
      },
      {
        lead: '实用建议',
        text: '照片类随便转；需要透明或精细文字就用 PNG / WebP。若必须转 JPG，质量别压得太低。',
      },
    ],
  },
  'gif-to-png': {
  ...
  'image-resizer': {
    title: 'Image Resizer：等比缩放前先了解',
    items: [
      {
        lead: '它在做什么',
        text: '按你设定的目标宽度等比缩放图片，高度按原比例自动变化，不会把图压扁或拉长。',
      },
      {
        lead: '放大 ≠ 更清晰',
        text: '超过原分辨率的放大只是插值（软件补像素）：画面不会变清晰，只会更糊、文件更大。不建议放大。',
      },
      {
        lead: '缩小能省体积',
        text: '把大图缩到合适宽度能明显减小体积，适合网页上传与分享；过度缩小则会丢失细节。',
      },
      {
        lead: '格式与质量',
        text: 'JPEG 通用、WebP 体积更小；有损格式质量越接近 1 越清晰、文件越大。需要透明请用 PNG / WebP。',
      },
      {
        lead: '实用建议',
        text: '网页素材用合适宽度（如 1280 / 640，甚至更小）；只缩不放，想小再配合较低质量。',
      },
    ],
  },
  'image-compressor': {
    title: 'Image Compressor：怎样真正把体积压下来',
    items: [
      {
        lead: '它在做什么',
        text: '在不改变图片尺寸的前提下，通过「换更高效格式 + 调低质量」来减小体积，适合贴网页、发消息、投稿上传。',
      },
      {
        lead: '质量是关键',
        text: '质量设为 1（100%）时几乎不压缩，体积基本不变；要真正减小，就把质量调低（如 0.7–0.8 开始试），越低越小但越糊。',
      },
      {
        lead: '格式选择',
        text: 'WebP 体积最小；JPEG 通用、体积适中；PNG 无损但通常最大。网页 / 照片用 WebP，需要兼容用 JPEG，需透明优先 WebP / PNG。',
      },
      {
        lead: '有损与无损',
        text: 'JPEG / WebP（有损）压缩会丢细节且不可逆；PNG（无损）只做无损压缩、体积压不了太多。想更小还得配合降分辨率。',
      },
      {
        lead: '实用建议',
        text: '先调低质量看体积与画质是否可接受；还嫌大就再用 Resizer 降低分辨率；重要原图记得留一份备份。',
      },
    ],
  },
};
